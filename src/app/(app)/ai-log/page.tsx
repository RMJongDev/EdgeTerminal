      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
            ...
            {data.aiLogs.map((log) => (
              <div key={log.id} className="grid gap-2 p-4 md:grid-cols-[170px_minmax(0,1fr)_120px_100px] md:items-center">
                ...
                <div className="text-sm text-muted-foreground">
                  <div>{log.summary}</div>
                  {typeof log.costSummary?.costEur === "number" ? (
                    <div className="mt-1 font-mono text-xs">EUR {log.costSummary.costEur}</div>
                  ) : null}
                </div>
        ...
        <Panel>
          ...
          <PanelBody className="space-y-3 text-sm">
            <div>
              <div className="text-muted-foreground">Filter model</div>
              <div className="font-mono">{runtime.filterModel}</div>
            </div>
            <div>
              <div className="text-muted-foreground">Analysis model</div>
              <div className="font-mono">{runtime.analysisModel}</div>
            </div>
