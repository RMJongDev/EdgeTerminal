...
Begeleidt Robin door het volledig invullen van een nieuw project-repo. Deze repo is een template die voor elk nieuw project gekloond wordt; vanaf dat moment is de kopie het project-repo en helpt deze skill alle kickstart-bestanden gestructureerd in te vullen via interviews en templates.
...
5. **Sluit de fase af met de checklist.** Elke fase heeft een checklist in [checklists/](checklists/). Loop hem expliciet langs voor je doorgaat.
6. **Edge cases:** geen logo → markeer in styleguide en vraag later op. Klant niet bereikbaar → markeer aannames met `> AANNAME:`. Onbekende tech-keuze → noteer als open punt in `Docs/Specs/voorstel-specs.md`.
...
- [references/](references/) — house-style en .claude/ setup-referentie
